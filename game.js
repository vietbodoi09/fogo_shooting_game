(async () => {
    ...
    let score = 0, elapsedTime = 0, gameOver = true;
    ...
    function draw() {
        ...
        bullets.forEach(b => ctx.fillRect(b.x, b.y, b.width * 1.5, b.height * 1.5)); // đạn to hơn
    ...
    function update(delta) {
        ...
        for (let i = bullets.length - 1; i >= 0; i--) {
            ...
            for (let j = enemies.length - 1; j >= 0; j--) {
                ...
                if (isColliding(b, e)) {
                    ...
                    // xóa đạn enemy của enemy này
                    enemyBullets = enemyBullets.filter(eb => !(eb.enemyId === e.id));
        ...
        for (let i = enemyBullets.length - 1; i >= 0; i--) {
            ...
            if (isColliding(b, ship)) { 
                ...
                sendTx('score', score, Math.floor(elapsedTime / 1000)); 
                break; 
            }
        }
        ...
        for (let i = enemies.length - 1; i >= 0; i--) {
            ...
            if (e.shootTimer > e.shootInterval) {
                e.shootTimer = 0;
                enemyBullets.push({ x: e.x + e.width / 2 - 3, y: e.y + e.height, width: 10, height: 16, speed: 2 + Math.random() * 2, enemyId: i });
            }
        ...
        difficulty = 1 + elapsedTime / 60000;
    }
    ...
    async function sendTx(action, value = null, elapsed = null) {
        if (!playerPubkey) { addLog('Register first'); return; }
        try {
            const body = { action, player: playerPubkey.toBase58() };
            if (action === 'register' && xHandle) body.xHandle = xHandle;
            if (action === 'score') { body.score = value; body.time = elapsed; }
            const resp = await fetch(PAYMASTER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
            const data = await resp.json();
            if (data.txSignature) addLog(`Tx: ${data.txSignature}`);
    ...
    async function fetchLeaderboard() {
        try {
            ...
            data.forEach(p => {
                ...
                li.innerHTML = `<span>${p.xHandle || p.player.slice(0,4)+'...'}</span>
                                <span>Score: ${p.score} | Time: ${p.time || 0}s</span>`;
