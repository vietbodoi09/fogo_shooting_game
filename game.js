(async () => {
    ...
    function addLog(msg){
        ...
        div.className='logEntry'; div.textContent=msg;
        logBox.prepend(div);
    ...
    function resetGame(){
        ...
        score=0; timeLeft=60000; gameOver=false; keysPressed={};
    ...
    function draw(){
        ...
        // Enemies
        enemies.forEach(e=>{
            if(e.img.complete) ctx.drawImage(e.img,e.x,e.y,e.width,e.height);
    ...
    registerBtn.addEventListener('click', async ()=>{
        const walletAddr = walletInput.value.trim(); const xHandleVal = xInput.value.trim();
        ...
        try{
            const resp=await fetch(PAYMASTER_URL,{method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({action:'register',player:playerPubkey.toBase58(),xHandle})});
            if(!resp.ok){ addLog('Paymaster error '+resp.status); return; }
            const data=await resp.json();
            if(data.txSignature){ addLog(`Registered ${xHandle} | Tx: ${data.txSignature}`); registerBtn.disabled=true; walletInput.disabled=true; xInput.disabled=true; resetGame(); } 
    ...
    async function fetchLeaderboard(){
        try{
            const resp = await fetch(PAYMASTER_URL+'/leaderboard'); if(!resp.ok){ addLog('Leaderboard fetch error'); return; }
    ...
    // Start game only after all images loaded
    const allImgs=[shipImg,...enemyImgs]; let loaded=0;
    function checkStart(){ loaded++; if(loaded===allImgs.length){ resetGame(); requestAnimationFrame(gameLoop); } }
    allImgs.forEach(img=>{ if(img.complete) checkStart(); else img.onload=checkStart; img.onerror=()=>{ console.error('Failed image '+img.src); checkStart(); } });
})();
