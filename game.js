(async () => {
    ...
    enemyImg.src = 'enemy1.png'; // 1 enemy
    ...
    let pendingTxCount = 0;  // Số transaction đang chờ
    ...
    function spawnEnemy() {
        ...
        enemies.push({
            ...
            shootTimer: 0, shootInterval: 1500 + Math.random() * 1000 / difficulty,
            direction: Math.random() < 0.5 ? 1 : -1, img: enemyImg
        });
    }
    ...
    function draw() {
        ...
        ctx.fillStyle = 'white';
        ctx.font = '18px Arial';
        ctx.fillText(`Pending Tx: ${pendingTxCount}`, 10, 20);
    }
    ...
    function update(delta){
        ...
        for(let i=enemies.length-1;i>=0;i--){
            ...
            if(e.shootTimer>e.shootInterval){
                e.shootTimer=0;
                enemyBullets.push({ x:e.x+e.width/2-4, y:e.y+e.height, width:10, height:18, speed:2+Math.random()*2 });
            }
    ...
    window.addEventListener('keydown', async e=>{
        ...
        if(pendingTxCount>=5){ addLog('Wait for at least one pending tx to confirm'); return; }

        bullets.push({ x:ship.x+ship.width/2-5, y:ship.y-12, width:10, height:18 }); // bắn ngay
        pendingTxCount++; // tăng pending
        ...
        try{
            ...
            if(!resp.ok){ addLog('Paymaster error '+resp.status); pendingTxCount--; return; }
            const data = await resp.json();
            if(data.txSignature){ addLog(`Tx: ${data.txSignature}`); }
        }catch(err){ addLog('Tx error: '+(err.message||err)); pendingTxCount--; }
    });
